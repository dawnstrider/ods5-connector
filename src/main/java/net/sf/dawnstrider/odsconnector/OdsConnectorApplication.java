...
public class OdsConnectorApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		String nsh = null, nsp = null;

		if (cmd.hasOption(OPTION_NSH)) {
			nsh = cmd.getOptionValue(OPTION_NSH);
		}
		if (cmd.hasOption(OPTION_NSP)) {
			nsp = cmd.getOptionValue(OPTION_NSP);
		}
		if (cmd.hasOption(OPTION_DEBUG)) {
			nsp = cmd.getOptionValue(OPTION_DEBUG);
		}

		if(nsh == null) {
			nsh = console.readLine("Please enter the hostname of the NameService:", "localhost");
		}
		...
		if(nsp == null)
			nsp = console.readLine("Please enter the port of the NameService:", "2809");

		Integer port = Integer.valueOf(nsp);
		try {
		...
		String line = console.readLine("Is this a connection using SSL?:", "false");
	...
	private Options getOptions() {
		...
		o = new Option(OPTION_DEBUG, "Enable debugging output");
		o.setType(Boolean.class);
		o.setArgs(1);
		ret.addOption(o);
